...
| Technical access to another CES service | Defines a `ServiceAccountRequest`, or a `ServiceAccountProducer` when offering access | Mediates credential creation and stores credentials in a Secret |
...
The Dogu custom resource is the desired state from the platform's perspective. The Helm release is the mechanism used to realize that state. Operators or administrators change the Dogu resource; they must not mutate the generated Helm release directly.

![Dogu V3 reconciliation and lifecycle](../images/multinode-lifecycle.svg)

The intended flow is:

1. a Dogu resource requests a Dogu and chart version;
...
5. CES controllers reconcile integration resources independently; and
6. status on the Dogu and integration resources reports progress or a failure.
...
The Dogu version is the chart `version`; `appVersion` records the packaged application's version. The two versions may differ. In the accepted target architecture, `dogu-upgrade.yaml` describes valid transitions between Dogu/chart versions and parameters the Dogu Operator needs to coordinate an upgrade. A chart can supply migration logic through Kubernetes-native mechanisms such as init containers or Helm hook Jobs. Migration logic must be:

- safe to retry after a Pod, Job or controller restart;
...
The operator must coordinate values and workload scaling when migration containers or hook Jobs need existing workloads or `ReadWriteOnce` volumes. Do not rely on V2 `pre-upgrade.sh` or `post-upgrade.sh` execution by the platform. A V3 chart supplies its migration implementation, but standalone Helm hooks are not yet a deployable V3 lifecycle contract.
...
The APIs do not share one readiness condition. Check their API-specific success conditions:
...
Use `metadata.generation`, condition `status`, `reason`, `message` and `observedGeneration` where the API provides it. A resource that exists but does not report its API-specific success conditions is not successfully integrated yet.
...
Do not hard-code a cluster-specific StorageClass. Define and test whether persistent data should be retained when the Dogu is uninstalled.
...
For HTTP routes, the Exposition identifies the target Service and port, the CES-relative path and an optional path rewrite. Design applications to work below a path such as `/my-dogu`; do not assume they run at `/`. Use TCP or UDP exposition only when the application protocol cannot run through the HTTP gateway. Requested external ports can conflict; inspect `LoadBalancerPortsAllocated` and its reason to determine whether allocation succeeded.
...
To participate in CES single sign-on, declare an [`AuthRegistration`](https://github.com/cloudogu/k8s-auth-registration-lib) (`k8s.cloudogu.com/v1`). It states the protocol (`CAS`, `OIDC` or `OAUTH`), consumer and optional logout URL and parameters.
...
An AuthRegistration only provisions the client-side integration. The application must still implement the selected authentication protocol, construct CES-path-aware callback/logout URLs and handle unavailable or rotated credentials.

## Two kinds of service account

### Kubernetes workload ServiceAccount

A Kubernetes `ServiceAccount` gives a Pod access to the Kubernetes API. Create one only when the application needs that access. Use a namespaced ServiceAccount, set `serviceAccountName` on the workload and grant only the required Role permissions. Avoid cluster-wide RBAC.

### CES service-account credentials

A [`ServiceAccountRequest`](https://github.com/cloudogu/k8s-serviceaccount-lib) (`k8s.cloudogu.com/v2`) asks another Dogu or component for technical credentials. The Service Account Operator provides them in a managed Secret.
...
- reload credentials or restart the workload after Secret rotation; and
...
Declare a [`WarpMenuEntry`](https://github.com/cloudogu/k8s-warp-menu-entry-lib) (`k8s.cloudogu.com/v1`) for each internal user-facing entry point. A Dogu may have no entry or multiple entries.

Each entry supplies German and English display names, a category key and a CES-relative path. External URLs are deliberately not allowed; they are controlled centrally by the platform. Set `disabled: true` to retain a declaration without rendering it. Use the `Ready` and `Visible` status conditions to diagnose an entry that does not appear.
...
Before handing over a chart or testing it in a partner CES, verify that:

- [ ] The chart renders with arbitrary release names and namespaces, and `values.schema.json` validates its values.
